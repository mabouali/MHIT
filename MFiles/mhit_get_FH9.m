function [fh9,Opt]=mhit_get_FH9(discharge,wYear,wYearList,Opt)
...
%% calculating mh14
thresh=Opt.dischargeExceedencePercentiles(Opt.dischargeExceedencePercentilesList==75);

nEventsPerYear=arrayfun(@(wy) sum(diff([0;(discharge(wYear==wy)>thresh)])>0), wYearList);

fh9=Opt.prefFun(nEventsPerYear);
end
